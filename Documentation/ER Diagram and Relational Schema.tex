...
\begin{document}
	...
	\item Rates: between customer and product for indicating rating
		\begin{enumerate}
			\item \underline{Product_ID, Customer_ID): Primary key
...
\begin{enumerate}
	\item Customer(\underline{customer_ID}, First_name, Last_name, House_number, Locality, City, Pincode, email_address, password)
	\item Customer_phone(\underline{customer_ID, phone_number})
	\item Product(\underline{product_ID}, Price, Category, Discount_Percentage, GST_Percentage)
	\item Product_Rating(\underline{Product_ID, Customer_ID, 
\end{enumerate}
\end{document}